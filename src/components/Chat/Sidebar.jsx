          ...
          groups.map((group) => (
            ...
            >
                <div className="flex items-center space-x-3 min-w-0">
                  <div className={`w-10 h-10 rounded-xl flex-shrink-0 flex items-center justify-center text-white font-bold text-lg shadow-sm overflow-hidden ${activeGroupId === group.id ? 'bg-indigo-600' : 'bg-gradient-to-br from-indigo-500 to-purple-600'}`}>
                    {group.groupPic ? (
              ...
              <div className="relative">
                ...
                {menuGroupId === group.id && (
                  <div className="absolute right-0 top-full mt-1 w-44 bg-white dark:bg-gray-700 rounded-xl shadow-2xl border border-gray-100 dark:border-gray-600 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    {group.members?.find(m => m.uid === userId)?.role === 'admin' && (
                      <button 
